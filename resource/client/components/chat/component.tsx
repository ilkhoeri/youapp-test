...
import { Account, AllChatProps, Message as MessageProp, MinimalAccount } from '@/resource/types/user';
...
import { ChatDisplay, ChatDisplayProps } from './chat-contents';

interface ChatAvatarsProps {
  data: (Chat & { users: MinimalAccount[] }) | null;
  otherUser: MinimalAccount;
}
...
interface ChatContainerProps extends ChatDisplayProps {
  accounts: Account[];
  chats: AllChatProps[];
...
export function ChatContainer(_props: ChatContainerProps) {
  ...
  const { accounts, chats, chat, message, messages, searchQuery, defaultLayout = desktopQuery ? [20, 32, 48] : [19, 31, 50], defaultCollapsed = false, navCollapsedSize } = _props;
  ...
  return (
    <ClientRender>
      <ResizablePanelGroup
        ...
        className="h-full max-xl:min-h-[176dvh] max-xl:max-h-[176dvh] xl:min-h-[520px] xl:max-h-[82dvh] items-stretch border rounded-2xl"
      >
        <ResizablePanel
        ...
        >
          <div className={cn('max-xl:col-span-full flex h-[52px] items-center justify-center', isCollapsed ? 'h-[52px]' : 'px-2')}>
            <ChatGroupSwitcher key={chat?.id} isCollapsed={isCollapsed} chats={chats} />
          </div>
        ...
        <ResizablePanel defaultSize={defaultLayout[1]} minSize={30}>
          <Tabs defaultValue="all" className="h-full">
            ...
            <Tabs.Panel value="all" className={classTabs.panel}>
              <ChatList accounts={accounts} items={chats} searchQuery={searchQuery} />
            </Tabs.Panel>
            <Tabs.Panel value="unread" className={classTabs.panel}>
              <ChatList accounts={accounts} items={chats.filter(item => !item.messages.map(ms => ms.seenIds.length > 0))} searchQuery={searchQuery} />
            </Tabs.Panel>
          </Tabs>
        </ResizablePanel>
        ...
        <ResizablePanel defaultSize={defaultLayout[2]} minSize={desktopQuery ? 55 : 30} className="relative">
          <ChatDisplay
            // chat={chats.find(item => item.id === mail.selected) || null}
