export function getAge(date: Date | string | string | undefined | null) {
  const birthDate = new Date(date ?? Date.now());
...
export function formatBirthDay(date: Date | string | null | undefined): string | undefined {
  if (!date) return;
